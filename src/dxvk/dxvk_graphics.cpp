...
namespace dxvk {
  ...
  : m_vkd(pipeMgr->m_device->vkd()), m_pipeMgr(pipeMgr) {
    ...
    if (vs  != nullptr) m_vs  = vs ->createShaderModule(m_vkd, slotMapping, false);
    if (tcs != nullptr) m_tcs = tcs->createShaderModule(m_vkd, slotMapping, false);
    if (tes != nullptr) m_tes = tes->createShaderModule(m_vkd, slotMapping, false);
    if (gs  != nullptr) m_gs  = gs ->createShaderModule(m_vkd, slotMapping, false);
    if (fs  != nullptr) m_fs  = fs ->createShaderModule(m_vkd, slotMapping, false);
    if (fs  != nullptr) m_fs2 = fs ->createShaderModule(m_vkd, slotMapping, true);
